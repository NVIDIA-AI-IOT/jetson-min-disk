...
??? note "Example commands of building custom RootFS for R32.7.2 (JetPack 4.6.12) on Host PC"
...
Follow this [section](https://docs.nvidia.com/jetson/archives/l4t-archived/l4t-3271/index.html#page/Tegra%20Linux%20Driver%20Package%20Development%20Guide/flashing.html) of L4T Developer Documentation to flash your Jetson.

??? note "Example command to flash Jetson Xavier NX production module from a Host PC"

    Make sure your target device (Jetson Xavier NX production module on a dev kit carrier board) is put in Force Recovery Mode. 
...
Once flashing is done, you can log into the Jetson using the username and password you used when running `l4t_create_default_user.sh` script.

You can check the disk usage nad the install package size using some of the commands introduced in [Analyze Disk Usage](./analysis.md) page

??? info "Example output of `df` and `dpkg` commands on Jetson set up with the minimal L4T (Rel-32.7.2)"
...
??? note "Example commands to build `minimal` flavor RootFS for R34.1.x (JetPack 5.0.x DP) on Host PC"
...
Follow [this section](https://docs.nvidia.com/jetson/archives/r34.1/DeveloperGuide/text/SD/FlashingSupport.html?highlight=flash%20sh#basic-flashing-script-usage) of Jetson Linux Developer Documentation to flash your Jetson.
...
??? info "Example output of `df` and `dpkg` commands on Jetson set up with the minimal L4T (Rel-34.1.1)"
